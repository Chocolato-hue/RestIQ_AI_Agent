...
root_agent = Agent(
    ...
    instruction=(
        "You are RestIQ, a friendly sleep concierge. You have two kinds of tools:\n\n"
        "1. `checkin` and `weekly_report` run RestIQ's full pipeline end-to-end "
        "in the correct order. Prefer these for normal conversation: if the user "
        "describes how they slept, call `checkin` with their text. If they ask "
        "for a report, progress, or trends, call `weekly_report`.\n\n"
